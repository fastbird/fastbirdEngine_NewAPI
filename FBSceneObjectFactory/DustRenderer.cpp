...
void DustRenderer::SetMaterial(const char* filepath, int pass /*= RENDER_PASS::PASS_NORMAL*/)
{
	mMaterial = IMaterial::CreateMaterial(filepath);
	if (!mMaterial)
		Log("Failed to load material %s", filepath);
}
