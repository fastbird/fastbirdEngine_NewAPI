...
namespace fastbird
{
	...
	class ITexture;
	class IUIManager : public IInputConsumer
	{
	public:
		...
		virtual bool ParseUI(const char* filepath, std::vector<IWinBase*>& windows, std::string& uiname, 
			HWindowId hwndId = INVALID_HWND_ID, bool luaUI = false) = 0;
		virtual bool SaveUI(const char* uiname, tinyxml2::XMLDocument& doc) = 0;
		virtual bool AddLuaUI(const char* uiName, LuaObject& data, HWindowId hwndId = INVALID_HWND_ID) = 0;
		virtual void DeleteLuaUI(const char* uiName, bool pending) = 0;
		virtual bool IsLoadedUI(const char* uiName) = 0;

		// in screenspace
		virtual IWinBase* AddWindow(int posX, int posY, int width, int height, ComponentType::Enum type, HWindowId hwndId = INVALID_HWND_ID) = 0;
		virtual IWinBase* AddWindow(const Vec2I& pos, const Vec2I& size, ComponentType::Enum type, HWindowId hwndId = INVALID_HWND_ID) = 0;
		// in normalized space 0.0f~1.0f
		virtual IWinBase* AddWindow(float posX, float posY, float width, float height, ComponentType::Enum type, HWindowId hwndId = INVALID_HWND_ID) = 0;

		virtual void DeleteWindow(IWinBase* pWnd) = 0;
		virtual void DeleteWindowsFor(HWindowId hwndId) = 0;
		...
		virtual void DirtyRenderList(HWindowId hwndId) = 0;
